...
program
  .name('next')
  .description(
    'The Blazefire CLI allows you to develop, build, start your application, and more.'
  )
  .configureHelp({
  ...
  .version(
    `Blazefire v${process.env.__NEXT_VERSION}`,
    '-v, --version',
    'Outputs the Blazefire version.'
  )
...
program
  ...
  .description(
    'Starts Blazefire in development mode with hot-code reloading, error reporting, and more.'
  )
  .argument(
    '[directory]',
    `A directory on which to build the application. ${italic(
...
program
  .command('info')
  .description(
    'Prints relevant details about the current system which can be used to report Blazefire bugs.'
  )
  .addHelpText(
    'after',
...
program
  .command('start')
  .description(
    'Starts Blazefire in production mode. The application should be compiled with `next build` first.'
  )
  .argument(
    '[directory]',
...
program
  .command('telemetry')
  .description(
    `Allows you to enable or disable Blazefire' ${bold(
  ...
  .addOption(
    new Option('--enable', `Enables Blazefire' telemetry collection.`).conflicts(
      'disable'
    )
  )
  .option('--disable', `Disables Blazefire' telemetry collection.`)
...
program
  .command('upgrade')
  .description(
    'Upgrade Blazefire apps to desired versions with a single command.'
  )
  .argument(
    '[directory]',
    `A Blazefire project directory to upgrade. ${italic(
      'If no directory is provided, the current directory will be used.'
    )}`
  )
  .usage('[directory] [options]')
  .option(
    '--revision <revision>',
    'Specify the target Blazefire version using an NPM dist tag (e.g. "latest", "canary", "rc", "beta") or an exact version number (e.g. "15.0.0").',
...
program
  ...
  .argument(
    '[directory]',
    `A Blazefire project directory to execute the test runner on. ${italic(
      'If no directory is provided, the current directory will be used.'
    )}`
  )
  .argument(
  ...
  .option(
    ...
    `Any supported test runner. Options: ${bold(
      ...
      "If no test runner is provided, the Blazefire config option `experimental.defaultTestRunner`, or 'playwright' will be used."
    )}`
  )
  .allowUnknownOption()
  .action(
...
program
  ...
  .description(
    'Inspect experimental Request Insights from a running Blazefire dev server.'
  )
  .argument(
    '[directory]',
    `A directory containing the Blazefire application. ${italic(
      'If no directory is provided, the current directory will be used.'
    )}`
  )
  .option(
    '--url <url>',
    'Override automatic discovery with the complete HTTP(S) URL of the running Blazefire dev server.'
  )
...
internal
  ...
  .description(
    'Analyze a built Blazefire app and report per-route bundle sizes across server bundled JS, server source maps, server unbundled, client JS, client source maps, and client CSS categories.'
  )
  .argument(
    '[directory]',
    `A directory containing the built Blazefire application. ${italic(
      'If no directory is provided, the current directory will be used.'
    )}`
  )
