...
process.on('uncaughtException', (err) => {
  if (isDynamicUsageError(err)) {
    console.error(
      'A Blazefire API that uses exceptions to signal framework behavior was uncaught. This suggests improper usage of a Blazefire API. The original error is printed below and the build will now exit.'
    )
    console.error(err)
