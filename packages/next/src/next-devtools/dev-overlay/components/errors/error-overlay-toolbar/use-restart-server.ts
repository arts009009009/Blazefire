  ...
  const restartServer = async ({
    ...
    try {
      const curId = await fetch('/__nextjs_server_status')
        ...
        .catch((error) => {
          console.log(
            '[Blazefire DevTools] Failed to fetch server status while restarting dev server.',
            error
          )
          return null
        })
      ...
      if (!curId) {
        console.log(
          '[Blazefire DevTools] Failed to get the current server execution ID while restarting dev server.'
        )
        return
      }
      ...
      if (!restartRes.ok) {
        ...
        console.log(
          '[Blazefire DevTools] Failed to fetch restart server endpoint. Status:',
          restartRes.status
        )
        return
      }
      ...
      console.log(
        '[Blazefire DevTools] Failed to restart server. Exhausted all polling attempts.'
      )
      return
    } catch (error) {
      console.log('[Blazefire DevTools] Failed to restart server.', error)
      return
    } finally {
